...
int RS232_PollComport(int comport_number, unsigned char *buf, int size)
{
  ...
  while(!flagStop && time(NULL) - oldTime <= 1)
  {
    result = read(Cport[comport_number], buf + beenRead, size - beenRead);

    if(result < 0)
    {
      if(errno == EINTR) continue;
      else if(errno == EAGAIN) flagStop = 1, result = 0;
      else flagStop = 1;
    }
    else
    {
      beenRead += result;
      if(beenRead == size) flagStop = 1;
    }
  }
...
int RS232_SendByte(int comport_number, unsigned char byte)
{
  ...
  while(!flagStop)
  {
    result = write(Cport[comport_number], &byte, 1);

    if(result < 0)
    {
      if(errno == EINTR) continue;
      if(errno == EAGAIN) flagStop = 1, result = 0;
      else flagStop = 1;
    }
    else flagStop = 1;
  }
...
int RS232_SendBuf(int comport_number, unsigned char *buf, int size)
{
  ...
  while(!flagStop)
  {
    result = write(Cport[comport_number], buf, size);
